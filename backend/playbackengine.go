...
func (p *playbackEngine) NowPlaying() mediaprovider.MediaItem {
	if p.nowPlayingIdx < 0 || p.getPlayQueueLength() == 0 || p.nowPlayingIdx >= p.getPlayQueueLength() || p.player.GetStatus().State == player.Stopped {
		return nil
	}
...
func (p *playbackEngine) handleOnTrackChange() {
	...
	if p.nowPlayingIdx < 0 || p.nowPlayingIdx >= p.getPlayQueueLength() {
		return
	}
