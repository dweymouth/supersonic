...
func (m *mpdMediaProvider) getAllAlbums() ([]*mediaprovider.Album, error) {
	...
	go func() {
		...
		err := m.server.withConn(func(conn *mpd.Client) error {
			...
			// Get simple list of unique album names (fast query)
			...
			// Process albums with parallel Find queries using connection pool
			const updateBatchSize = 50 // Update cache every 50 albums
			const parallelQueries = 5  // Number of parallel Find queries
			...
			for w := 0; w < parallelQueries; w++ {
				...
				go func() {
					...
					for albumName := range albumChan {
						// Use connection pool's withConn for each query
						err := m.server.withConn(func(workerConn *mpd.Client) error {
							...
							if count%updateBatchSize == 0 || count == totalNames {
								// Convert current albumMap to sorted slice
								albumMapMu.Lock()
...
func (m *mpdMediaProvider) getAllArtists() ([]*mediaprovider.Artist, error) {
	...
	now := time.Now().Unix()
	if m.artistsCached != nil {
		m.artistsFetchMu.Lock()
		isFetchInProgress := m.artistsFetchInProgress
		m.artistsFetchMu.Unlock()

		if (now-m.artistsCachedAt) < artistsCacheValidDurationSeconds || isFetchInProgress {
	...
	// Use mutex to check if fetch is needed
	m.artistsFetchMu.Lock()

	// Check if another goroutine already started the fetch
	...
	if m.artistsCached != nil && (now-m.artistsCachedAt) < artistsCacheValidDurationSeconds {
		artistsCopy := make([]*mediaprovider.Artist, len(m.artistsCached))
		copy(artistsCopy, m.artistsCached)
		m.artistsFetchMu.Unlock()
		return artistsCopy, nil
